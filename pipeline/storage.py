...
from pipeline.postgres_storage import PostgresVectorStore
...
class VectorStore:
    ...
    def query_similar_chunks(self, query_embedding: Sequence[float], *, document_ids: str | Sequence[str] | None = None, tags: Sequence[str] | None = None, min_importance: float | None = None, top_k: int = 5) -> List[Tuple[str, int, ChunkEmbedding, float]]:
        ...
        logger.info("Query similar chunks for doc=%s tags=%s min_importance=%s -> %s hits", document_id, tags, min_importance, len(top_results))
        return top_results
...
def open_store(db_path: Union[Path, str]):
    ...
    store = PostgresVectorStore(str(db_path)) if _is_postgres(db_path) else VectorStore(Path(db_path))
    try:
        yield store
    finally:
        store.close()
