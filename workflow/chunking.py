...
from pipeline.types import ChunkCandidate, Paragraph
...
def adaptive_chunk_paragraphs(paragraphs: Iterable[Paragraph], max_tokens: int = 220, overlap: int = 40) -> List[ChunkCandidate]:
    all_tokens: List[str] = []
    ...
    while start < total_tokens:
        ...
        if _should_extend(chunk_tokens) and end < total_tokens:
...
def enforce_chunk_quality(chunks: List[ChunkCandidate], min_tokens: int = 40, max_tokens: int = 260) -> List[ChunkCandidate]:
    filtered: List[ChunkCandidate] = []
    carry: ChunkCandidate | None = None

    for chunk in chunks:
        if _chunk_noisy(chunk.text):
            continue

        current = chunk

        if current.tokens > max_tokens:
            filtered.extend(split_long_chunk(current, max_tokens))
            continue
